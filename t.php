...
function turnUrlIntoHyperlink($string) {
    ...
    return preg_replace_callback($pattern, function ($matches) {
        ...
        if (!isset($parsedUrl['scheme']) || !in_array(strtolower($parsedUrl['scheme']), ['http', 'https', 'ftp'], true)) {
            return $display;
        }

        return '<a href="' . safe_html_output($href) . '">' . safe_html_output($display) . '</a>';
    }, $string);
}
